 ...
 *     multibyte values (via {@link google.common.hash.Hasher}), but this is merely a convenience; these are
 ...
 *     represents a hash code as an instance of {@link google.common.hash.HashCode}.
 *
 * <li><b>pure function:</b> the value produced must depend only on the input bytes, in
 *     the order they appear. Input data is never modified. {@link google.common.hash.HashFunction} instances
 ...
 * {@link google.common.hash.Hasher}. Obtain a new hasher from the hash function using {@link #newHasher},
 * "push" the relevant data into it using methods like {@link google.common.hash.Hasher#putBytes(byte[])},
 * and finally ask for the {@code HashCode} when finished using {@link google.common.hash.Hasher#hash}. (See
 * an {@linkplain #newHasher example} of this.)
 *
 * <p>If all you want to hash is a single byte array, string or {@code long} value, there
 * are convenient shortcut methods defined directly on {@link google.common.hash.HashFunction} to make this
 * easier.
 *
 * <p>Hasher accepts primitive data types, but can also accept any Object of type {@code
 * T} provided that you implement a {@link google.common.hash.Funnel Funnel<T>} to specify how to "feed" data
 * from that object into the function. (See {@linkplain google.common.hash.Hasher#putObject an example} of
 * this.)
 *
...
public interface HashFunction {
   ...
   * using the given {@link java.nio.charset.Charset}. The implementation <i>might</i> perform better than its
