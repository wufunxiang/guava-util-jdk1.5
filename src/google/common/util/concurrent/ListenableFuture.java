...
/**
 * A {@link java.util.concurrent.Future} that accepts completion listeners.  Each listener has an
 * associated executor, and it is invoked using this executor once the future's
 * computation is {@linkplain java.util.concurrent.Future#isDone() complete}.  If the computation has
 ...
 * derived {@code Future}, as in {@link google.common.util.concurrent.Futures#allAsList(Iterable)
 ...
 * {@link google.common.util.concurrent.Futures#addCallback Futures.addCallback}.) Still, direct {@code
 ...
 * <li>If they are manually filled in by a call to {@link java.util.concurrent.FutureTask#set} or a
 * similar method, create a {@link SettableFuture} instead. (Users with more
 * complex needs may prefer {@link google.common.util.concurrent.AbstractFuture}.)
 * </ul>
 *
...
public interface ListenableFuture<V> extends Future<V> {
  /**
   * Registers a listener to be {@linkplain java.util.concurrent.Executor#execute(Runnable) run} on
   * the given executor.  The listener will run when the {@code Future}'s
   * computation is {@linkplain java.util.concurrent.Future#isDone() complete} or, if the computation
