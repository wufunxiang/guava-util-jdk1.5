...
import static google.common.collect.Ranges.create;
 ...
 * google.common.collect.DiscreteDomain} can be provided to the {@link #asSet asSet} method.
 *
 ...
 * <tr><td>{@code (a..+鈭�} <td>{@code {x | x > a}}      <td>{@link Ranges#greaterThan greaterThan}
 * <tr><td>{@code [a..+鈭�} <td>{@code {x | x >= a}}     <td>{@link Ranges#atLeast atLeast}
 * <tr><td>{@code (-鈭�.b)} <td>{@code {x | x < b}}      <td>{@link Ranges#lessThan lessThan}
 * <tr><td>{@code (-鈭�.b]} <td>{@code {x | x <= b}}     <td>{@link Ranges#atMost atMost}
 * <tr><td>{@code (-鈭�.+鈭�}<td>{@code {x}}              <td>{@link Ranges#all all}
 * </table></blockquote>
 *
 ...
 *     100."
 * <li>When evaluated as a {@link google.common.base.Predicate}, a range yields the same result as invoking {@link
 *     #contains}.
...
public final class Range<C extends Comparable> implements Predicate<C>, Serializable {
  ...
  /**
   * Returns the type of this range's lower bound: {@link google.common.collect.BoundType#CLOSED} if the range includes
   * its lower endpoint, {@link google.common.collect.BoundType#OPEN} if it does not.
   *
   * @throws IllegalStateException if this range is unbounded below (that is, {@link
   *     #hasLowerBound()} returns {@code false})
   */
  ...
  /**
   * Returns the type of this range's upper bound: {@link google.common.collect.BoundType#CLOSED} if the range includes
   * its upper endpoint, {@link google.common.collect.BoundType#OPEN} if it does not.
   *
   * @throws IllegalStateException if this range is unbounded above (that is, {@link
   *     #hasUpperBound()} returns {@code false})
   */
  ...
  /**
   * Equivalent to {@link #contains}; provided only to satisfy the {@link google.common.base.Predicate} interface. When
   ...
   * google.common.base.Equivalence equivalence relation} as it is not transitive.
   */
   ...
   *
   * <p>Like {@link #intersection(google.common.collect.Range) intersection}, this operation is commutative, associative
  ...
  /**
   * Returns an {@link google.common.collect.ContiguousSet} containing the same values in the given domain
   * {@linkplain google.common.collect.Range#contains contained} by this range.
   *
   ...
   * performed efficiently, but others (such as {@link java.util.Set#hashCode} or {@link
   * java.util.Collections#frequency}) can cause major performance problems.
   *
   ...
   * <li>[start..end)
   * <li>[start..+鈭�
   * <li>(-鈭�.end) (only if type {@code C} is unbounded below)
   * <li>(-鈭�.+鈭� (only if type {@code C} is unbounded below)
   * </ul>
   */
